...
public class GroupConcatDistinctUDAF extends UserDefinedAggregateFunction {
	...
	public void update(MutableAggregationBuffer buffer, Row input) {
		...
		for (String string : split) {
			...
			if(oldVS == null) {
				...
				bufferMonitorInfo = StringUtils.setFieldInConcatString(bufferMonitorInfo, "\\|", 
						monitorId, Integer.parseInt(oldVS)+addNum+"");
			}
	...
	public void merge(MutableAggregationBuffer buffer1, Row buffer2) {
		...
		/*------------ 替代下面的for循环  --------------------------*/
		/**
		 * 该api，传入一个字符串，先按 |切割，然后遍历切割得到的数组，内部每个字符串再按‘=’切分，[0]为key,[1]为val
		 * monitor_id1 100
		 */
		Map<String, String> buffer1Map = StringUtils.getKeyValuesFromConcatString(bufferMonitorInfo1, "\\|");
		Map<String, String> buffer2Map = StringUtils.getKeyValuesFromConcatString(bufferMonitorInfo2, "\\|");
		
		for (Entry<String, String> buffer2Entry : buffer2Map.entrySet()) {
			String key = buffer2Entry.getKey();
			if(buffer1Map.containsKey(key)) {
				int val = Integer.parseInt(buffer1Map.get(key)) + Integer.parseInt(buffer2Entry.getValue());
				buffer1Map.put(key, val + "");
			} else {
				buffer1Map.put(key, buffer2Entry.getValue());
			}
		}
		
		StringBuffer sb = new StringBuffer();
		for (Entry<String, String> buffer1Entry : buffer1Map.entrySet()) {
			sb.append("|").append(buffer1Entry.getKey()).append("=").append(buffer1Entry.getValue());
		}
		buffer1.update(0, sb.substring(1).toString()); 
		/*------------ 替代下面的for循环  --------------------------*/
