...
//				.registerKryoClasses(new Class[]{SpeedSortKey.class})
		 ...
		 * 如果底层相关配置项为true则为master,如果为false，则放回conf.setMaster(local);
		 */
...
//			SparkUtils.mockData(sc, sqlContext);
		 ...
		 * '{\"startDate\":[\"2018-01-01\"],\"endDate\":[\"2018-01-01\"],\"topNum\":[\"5\"],\"areaName\":[\"海淀区\"]}'
		 */
	 ...
	 */
	private static void getMonitorDetails(JavaSparkContext sc , 
			final long taskId,List<String> top5MonitorIds, JavaPairRDD<String, Row> monitor2DetailRDD) {
		/**
			...
			public void call(Tuple2<String, Iterable<Row>> tuple) throws Exception {
				...
				for (Row row : top10Cars) {
					 topNMonitorDetailInfos.add(
							 new TopNMonitorDetailInfo(taskId, row.getString(0), 
	...
	private static List<String> speedTopNMonitor(JavaPairRDD<String, Iterable<Row>> groupByMonitorId) {
		   ...
		   */
		  JavaPairRDD<SpeedSortKey, String> speedSortKey2MonitorId = groupByMonitorId.mapToPair(
	 ...
	 */
	private static JavaPairRDD<String, String> aggreagteByMonitor(
			JavaPairRDD<String, Iterable<Row>>  monitorId2RowRDD) {
		/**
		...
		JavaPairRDD<String, String> monitorId2CameraCountRDD = monitorId2RowRDD.mapToPair(
			...
			public Tuple2<String, String> call(Tuple2<String, Iterable<Row>> tuple) throws Exception {
				 ...
				 * list.count方便知道此monitor下对应多少个cameraId
				 */
	...
	private static void saveMonitorState(Long taskId,Accumulator<String> monitorAndCameraStateAccumulator) {
		 ...
		 * 这里面只有一条记录
		 */
		MonitorState monitorState = new MonitorState(taskId, normalMonitorCount, 
				normalCameraCount, abnormalMonitorCount, abnormalCameraCount, abnormalMonitorCameraInfos);
	...
	private static void getTopNDetails(
		...
		/**
		 * 优化点：
		 * 因为topNMonitor2CarFlow 里面有只有5条数据，可以将这五条数据封装到广播变量中，
		 * 然后遍历monitor2DetailRDD ，每遍历一条数据与广播变量中的值作比对。
		 */
		topNMonitor2CarFlow.join(monitor2DetailRDD).mapToPair(
			...
			public void call(Iterator<Tuple2<String, Row>> t) throws Exception {
				...
				while (t.hasNext()) {
					...
					Row row = tuple._2;
					TopNMonitorDetailInfo m = new TopNMonitorDetailInfo(taskId, row.getString(0), 
		...
		JavaRDD<String> topNMonitorCarFlow = topNMonitor2CarFlow.map(
		...
		//过滤
		JavaPairRDD<String, Row> filterTopNMonitor2CarFlow = monitor2DetailRDD.filter(
		...
		filterTopNMonitor2CarFlow.foreachPartition(new VoidFunction<Iterator<Tuple2<String,Row>>>() {
			...
			public void call(Iterator<Tuple2<String, Row>> t)
				...
				while(t.hasNext()){
					Tuple2<String, Row> tuple = t.next();
					Row row = tuple._2;
					TopNMonitorDetailInfo m = new TopNMonitorDetailInfo(
							taskId, row.getString(0), row.getString(1), row.getString(2), 
		...
		JavaPairRDD<Integer, String> carCount2MonitorId = joinResultRDD.mapPartitionsToPair(
			...
			public Iterable<Tuple2<Integer, String>> call(
				...
				while (iterator.hasNext()) {
					 ...
					 * 从标准数据拼接的字符串中获取摄像头数
					 */
					int standardCameraCount = Integer.parseInt(StringUtils.getFieldFromConcatString(standardCameraInfos, 
							"\\|", Constants.FIELD_CAMERA_COUNT));
